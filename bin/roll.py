...
async def packs_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.effective_chat.send_message("Доступные паки:",
                                             reply_markup=generate_available_packs_keyboard(User.get(update.effective_user)))
...
async def roll_new(update: Update, _: ContextTypes.DEFAULT_TYPE, **kwargs):
    ...
    if not user.rolls_available or user.rolls_available.get(pack_type) <= 0:
        ...
        await update.effective_chat.send_message(f"У вас не осталось доступных круток!\n\nОсталось до бесплатной: "
                                                 f"{time_left}")
        return
