...
class FilterMarketMyOffersButton(filters.MessageFilter):
    def filter(self, message: Message) -> bool:
        return message.text is not None and re.search("Мoи предложения", message.text) is not None
...
class FilterMarketOffersButton(filters.MessageFilter):
    def filter(self, message: Message) -> bool:
...
class FilterSettingsButton(filters.MessageFilter):
    def filter(self, message: Message) -> bool:
        return message.text is not None and re.search("Настройки", message.text) is not None
...
settings_button_filter = FilterSettingsButton()
